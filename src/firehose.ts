...
export class Firehose extends EventEmitter {
  ...
  private async parseMessage(data: WS.RawData) {
    ...
    if (!frame.header.t || !frame.body || typeof frame.body !== "object") {
      throw new Error(
        "Invalid frame structure: " + JSON.stringify(frame, null, 2)
      );
    }
    ...
    if (frame.header.t === "#commit") {
      ...
      if (
        !("blocks" in frame.body) ||
        !(frame.body.blocks instanceof Uint8Array)
      ) {
        return {
      ...
      const commit = frame.body as any;
      ...
      for (const op of commit.ops) {
        ...
        if (action === "create" || action === "update") {
          if (!op.cid) return;
          const recordBlocks = await car.get(op.cid);
          if (!recordBlocks) return;
          op.record = cborDecode(recordBlocks.bytes);
        }
      }
